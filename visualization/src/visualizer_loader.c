...
static int has_value_delimiter(const char *cursor)
{
    if (cursor == NULL) {
        return 0;
    }

    return (*cursor == ',') || (*cursor == '}') || (*cursor == ']') || (*cursor == '\0');
}
...
static int parse_quoted_value(const char *object_start, const char *key, char *out, size_t out_size)
{
    ...
    if (memcpy(out, value_start, length) != out) {
        return 0;
    }
    out[length] = '\0';
    return 1;
...
static int parse_uint_value(const char *object_start, const char *key, unsigned int *out)
{
    ...
    if ((errno != 0) || (endptr == value_start) || (parsed > UINT_MAX)) {
        return 0;
    }

    value_end = skip_ws(endptr);
    if (!has_value_delimiter(value_end)) {
        return 0;
    }
    ...
    while (cursor < array_end) {
        ...
        while (tick_cursor < ticks_array_end) {
            ...
            if (parse_tick_object(obj_start,
                          &scenarios[scenario_count].ticks[scenarios[scenario_count].tick_count]) ==
                0) {
                return 0;
            }
