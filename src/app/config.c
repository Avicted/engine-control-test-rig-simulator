...
static int32_t parse_float_field(const char *buffer, const char *key, float *value_out)
{
    ...
    if ((buffer == (const char *)0) || (key == (const char *)0) || (value_out == (float *)0))
    {
        return 0;
    }

    (void)snprintf(pattern, sizeof(pattern), "\"%s\"", key);
    key_pos = strstr((char *)buffer, pattern);
    if (key_pos == (char *)0)
    {
        return 0;
    }
...
static int32_t parse_uint_field(const char *buffer, const char *key, uint32_t *value_out)
{
    ...
    if ((buffer == (const char *)0) || (key == (const char *)0) || (value_out == (uint32_t *)0))
    {
        return 0;
    }
...
static int32_t validate_only_known_keys(const char *buffer)
{
    ...
    cursor = buffer;
    while ((cursor = strchr(cursor, '"')) != (const char *)0)
    {
        char key[64];
                                        ...
                                        uint32_t error_message_size)
{
    ...
    if ((path == (const char *)0) || (calibration_out == (ControlCalibration *)0) || (error_message == (char *)0) ||
